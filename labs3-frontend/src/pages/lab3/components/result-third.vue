<template>
  <div class="q-pa-md flex column g-xl">
    <div class="text-h6 text-primary">
      Определите пересечение отношений предпочтений (без учета весов)
    </div>
    <q-table
      flat
      bordered
...
onMounted(() => {
  ...
  const userAnswers = localStorage.getItem('user-answers-3');
  if (userAnswers) {
...
watch(
  rows,
  () => {
    localStorage.setItem('user-answers-3', JSON.stringify(rows.value));
  },
  {
    deep: true,
  }
...
const sendResult = async () => {
  ...
  const resAnswer = await labStore.sendIntersections({ matrix: res });
  if (resAnswer.data) {
