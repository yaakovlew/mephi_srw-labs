<template>
  <div class="q-pa-md flex column g-xl">
    <div class="text-h6 text-primary">
      Постройте множество недоминируемых альтернатив Q1 (без учета весов)
    </div>
    <q-table
      flat
      bordered
...
onMounted(() => {
  ...
  const userAnswers = localStorage.getItem('user-answers-4');
  if (userAnswers) {
...
watch(
  rows,
  () => {
    localStorage.setItem('user-answers-4', JSON.stringify(rows.value));
  },
  {
    deep: true,
  }
...
const sendResult = async () => {
  ...
  const res = await labStore.sendNonDominated({ set: answ[0] });
  if (res.data) {
